...
def inference(
...
):  # Rename to gemma4 specific module, to accomodate other open-source models
    """
    ...
    outputs = model.generate(
        **inputs, max_new_tokens=1024, temperature=1.0, top_p=0.95, top_k=64
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--file",
        required=True,
        help="Path to json containing single-turn prompts",
    )
    parser.add_argument(
        "-m",
        "--model_id",
    ...
    parser.add_argument(
        "-r", "--run_id", required=False, help="Peft adapter for finetuned variant"
    )
    parser.add_argument(
        "-s",
        "--save",
        type=bool,
        default=False,
        help="Saves the model response to a local path",
    )
    parser.add_argument(
        "-sp",
        "--system_prompt",
        type=str,
        help="System Prompt for model instance",
        default="Ensure you end the conversation, after the assistant speaks."
    ...
    if args.file:
        ...
        with open(args.file, "r") as ins:
            ...
            for item in obj["messages"]:
                prompt = f"<|turn|> system |<think>| {args.sp} \n"
